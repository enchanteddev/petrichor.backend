...
def upsertItem(request):
    ...
    if data:
        try:
            ...
            item.save()
            print("pp")
            return r200("Item updated/added")
...
def addAddress(request):
    ...
    if data:
        try:
            ...
            address=Address.objects.create(userId=userId,address=address,pincode=pinCode)
            address.save()
    ...
    if data:
        try:
            ...
            for address in db_adressess:
                allAddresses.append({"address":address["address"],
        ...
        try:
            ...
            if PaymentTable.objects.filter(transactionId=tr_Id).first():
                ...
                if userId.endswith("@smail.iitpkd.ac.in"):
                    verified=True
                payment=PaymentTable.objects.create(transactionId=tr_Id,itemId=itemId,userId=userId,verified=verified)
                payment.save()
...
def filterTransaction(request):
    ...
    if data:
        try:
            ...
            for tr in trs:
                allTrs.append({"transactionId":tr["transactionId"],"userId":tr["userId"]})

            return Response({
                "status":200,"transactions":allTrs
            })
